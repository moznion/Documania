...
use Class::Accessor::Lite (
    new => 0,
    rw  => [qw/packages routes base_dir template syntax/],
);
...
sub new {
    my ($class, $dispatcher_class, $args) = @_;

    my $dispatcher_name = $args->{dispatcher_name} || 'Router::Boom';
    $dispatcher_name = 'Documania::Dispatcher::' . $dispatcher_name;
    eval "require $dispatcher_name"; ## no critic

    my ($packages, $routes) = $dispatcher_name->analyze_routes($dispatcher_class);
